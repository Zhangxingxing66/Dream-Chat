...
const ContextProvider = (props) => {
  ...
  const regenerateMessage = useCallback(async (messageIndex) => {
    ...
    // 截断到该 AI 消息之前（保留 user 消息，移除旧的 AI 回复）
    ...
    try {
      ...
      await streamParser.fetchStream(
        ...
        (error) => {
          ...
          const errorMessages = messagesWithAI.map(msg =>
            msg.id === aiMessage.id ? { ...msg, status: 'failed', content: fullContent || '生成失败，请重试' } : msg
          );
          setMessages(errorMessages);
          updateSessionMessages(errorMessages, { isGenerating: false });
          setIsGenerating(false);
        ...
        () => {
          const completedMessages = messagesWithAI.map(msg =>
            msg.id === aiMessage.id ? { ...msg, status: 'completed', content: fullContent } : msg
          );
          setMessages(completedMessages);
          updateSessionMessages(completedMessages, { resultData: fullContent, isGenerating: false });
          setIsGenerating(false);
      ...
      console.error('Regenerate error:', error);
      const errorMessages = messagesWithAI.map(msg =>
        msg.id === aiMessage.id ? { ...msg, status: 'failed', content: '生成失败，请重试' } : msg
      );
      setMessages(errorMessages);
      updateSessionMessages(errorMessages, { isGenerating: false });
      setIsGenerating(false);
    }
  }, [isGenerating, messages, updateSessionMessages, scrollToBottom]);
